...
class Cache{
  ...
  //|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  int base = block * (4 * solve.col);
  //if (dirty[i] == 1) writeBlock(i);
  tag[i] = tagelement;
  filled[i] = 1;
  dirty[i] = 0;
  if (policy == 1 || policy == 2) status[i] = solve.col - 1;
  for (int j = 0; j < solve.col; j++) {
    if(INSorMEM==false)
...
int getnumber(int address,bool cacheop,int data) {
  ...
  int block = address ~/ (4 * solve.col);
  int offset = address & ((4 * solve.col) - 1);
  offset = offset >> 2;
  int index = block & (solve.row - 1);
  int tagelement = block ~/ (solve.row);
  ///////////////1:-     DIRECT MAPPING     //////////////
  if (mapping == 0) {
  ...
  //fullyassociative
    ...
    int statusBit = 0;
    for (int i = 0; i < solve.row; i++) {
      if (filled[i] == 1) statusBit++;
      if (tag[i] == block) {
        this.hits++;//|||||||||||||||||||||||||
        if (policy == 1) {
          statusBit = 0;
          for (int j = 0; j < solve.row; j++) {
    ...
    if (policy == 0) {
      ...
      int i = random.nextInt(solve.row);
  ...
  ///////////////////////////////////////////SET associative////////////////////////////////////////
    ...
    int set = way * (block % (solve.row ~/ way));
    tagelement = block ~/ (solve.row ~/ way);
    ...
    if (policy == 0) {
      ...
      if(INSorMEM==true)solve.INSconflict.add((tag[i]*(solve.row ~/ way))+set);
      else solve.MEMconflict.add((tag[i]*(solve.row ~/ way))+set);
    ...
    //////2:-        LRU policy or FIFO policy
      ...
      if(INSorMEM==true)solve.INSconflict.add((tag[index]*(solve.row ~/ way))+set);
      else solve.MEMconflict.add((tag[index]*(solve.row ~/ way))+set);
...
void main() {
  solve.datacache=new Cache(solve.row,solve.col);
  solve.inscache=new Cache(solve.row,solve.col);
  if (solve.row % solve.inscache.way != 0 || solve.inscache.way <= 0 || solve.row % solve.datacache.way != 0 || solve.datacache.way <= 0) {
