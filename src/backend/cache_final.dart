...
class Cache{
    ...
    mapping = 0,
    way = 2;
    var cache = List.generate(row, (i) => List.filled(col, 0, growable: false),growable: false);
    var dirty = List.generate(row, (i) => 0, growable: false);
    var status = List.generate(row, (i) => 0, growable: false);
    var filled = List.generate(row, (i) => 0, growable: false);
    var tag = List.generate(row, (i) => -1, growable: false);
    int accesses=0,
     hits=0,
     misses=0,
     coldmisses=0,
  ...
  this.misses++;
  totalstalls+=penalty;
  if(INSorMEM==true && INScold.contains(tagelement)==false)this.coldmisses++;
  else if(INSorMEM==false && MEMcold.contains(tagelement)==false)this.coldmisses++;
  else{
     if(INSorMEM==true && INSconflict.contains(tagelement)==true && this.mapping!=1)this.conflictmisses++;
     else if(INSorMEM==false && MEMconflict.contains(tagelement)==true && this.mapping!=1)this.conflictmisses++;
     else this.capacitymisses++;
  }
  if(INSorMEM==true)INScold.add(tagelement);
  else MEMcold.add(tagelement);
  //|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  int base = block * (4 * col);
  //if (dirty[i] == 1) writeBlock(i);
  tag[i] = tagelement;
  filled[i] = 1;
  dirty[i] = 0;
  if (policy == 1 || policy == 2) status[i] = col - 1;
  for (int j = 0; j < col; j++) {
    if(INSorMEM==false)
    cache[i][j] = MEM[base + (4 * j)] ?? 0;
    else
    cache[i][j] = INS[base + (4 * j)] ?? 0;
  }
}
...
int getnumber(int address,bool cacheop,int data) {
  ...
  int block = address ~/ (4 * col);
  int offset = address & ((4 * col) - 1);
  offset = offset >> 2;
  int index = block & (row - 1);
  int tagelement = block ~/ (row);
  ///////////////1:-     DIRECT MAPPING     //////////////
  if (mapping == 0) {
    if (tag[index] != tagelement) {

      if(INSorMEM==true)INSconflict.add((tag[index]*row)+index);
      else MEMconflict.add((tag[index]*row)+index);
  ...
  //fullyassociative
    ...
    if (policy == 0) {
      ...
      if(INSorMEM==true)INSconflict.add(tag[i]);
      else MEMconflict.add(tag[i]);
    ...
    ///// 2:-        LRU policy or FIFO policy
      ...
      if(INSorMEM==true)INSconflict.add(tag[index]);
      else MEMconflict.add(tag[index]);
  ...
  ///////////////////////////////////////////SET associative////////////////////////////////////////
    ...
    int set = way * (block % (row ~/ way));
    tagelement = block ~/ (row ~/ way);
    ...
    if (policy == 0) {
      ...
      if(INSorMEM==true)INSconflict.add((tag[i]*(row ~/ way))+set);
      else MEMconflict.add((tag[i]*(row ~/ way))+set);
    ...
    //////2:-        LRU policy or FIFO policy
      ...
      if(INSorMEM==true)INSconflict.add((tag[index]*(row ~/ way))+set);
      else MEMconflict.add((tag[index]*(row ~/ way))+set);
...
//||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  ...
  void run_riscvsim(File f){
    while(running){
      fetch();
      if(stop>0){count+=stop;stop=0;continue;}
      decode_p();
      execute();
      memory();
      if(stop>0){count+=stop;stop=0;continue;}
      ...
      // print(ma_wb);
       //print(RF);
       //if(count>100000){ print(MEM);exit(0);}
      //print(tag);
      count++;
      transfer();
    }
...
void main() {
  if (row % inscache.way != 0 || inscache.way <= 0 || row % datacache.way != 0 || datacache.way <= 0) {
    print("invalid inputs to cache.");
    exit(0);
  }
  inscache.INSorMEM=true;
